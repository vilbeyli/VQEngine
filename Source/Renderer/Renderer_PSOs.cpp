...
void VQRenderer::LoadBuiltinPSOs()
{
	...
	{
		SCOPED_CPU_MARKER("Compile");
		mLightingPSOs.Compile(*this);
...
void VQRenderer::FLightingPSOs::GatherPSOLoadDescs(const std::unordered_map<RS_ID, ID3D12RootSignature*>& mRootSignatureLookup)
{
...
void VQRenderer::FLightingPSOs::Compile(VQRenderer& Renderer)
{
	SCOPED_CPU_MARKER("ForwardLightingPSOs");
