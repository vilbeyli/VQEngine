...
FBlob CompileFromSource(const FShaderStageCompileDesc& ShaderStageCompileDesc, std::string& OutErrorString)
{
	...
	else
	{
		...
		Microsoft::WRL::ComPtr<IDxcCompiler3> DXC_compiler3; // IDxcCompiler3 is the latest as of writing this code (April 2021)
		...
		Microsoft::WRL::ComPtr<IDxcVersionInfo> DXC_versionInfo; // ?
...
bool CompileFromCachedBinary(const std::string& ShaderBinaryFilePath, FBlob& Blob, bool bSM6, std::string& errMsg)
{
	...
	if (bSM6)
	{
		...
		if (!ValidateDXCBlob(Blob.pBlobDxc.Get()))
		{
			errMsg = "Failed to validate shader blob for " + ShaderBinaryFilePath;
		}
